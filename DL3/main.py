...
'''
    hist() 함수 안에 x축 지정 (가져올 칼럼)
        ...
        - bins : x축 몇개의 막대로 쪼개어 줄것인지 정함
    => plasma 수치가 높아질수록 당뇨인 경우가 많음
'''
# plt.hist(x=[df.plasma[df.diabetes == 0], df.plasma[df.diabetes == 1]], bins=30, histtype='barstacked', label=['nomal', 'diabetes'])
# plt.legend()
# plt.show()
...
'''
    ...
    평균이나 중앙값으로 대치하거나, 이상치를 제거 등 과정 등이 전처리에 포함가능(성능 높임)
'''
